...
// ---------- Paid-traffic funnel (7d, from Meta events): stages, leaks, fixes ----------
export function funnelRead(days) {
  const w7 = lastN(days, 7)
  const p7 = lastN(days, 7, 7)
  const sum = (w, k) => w.reduce((s, d) => s + num(d[k]), 0)
  const cur = { clicks: sum(w7, 'link_clicks'), atc: sum(w7, 'adds_to_cart'), ic: sum(w7, 'initiate_checkout'), pur: sum(w7, 'purchases') }
  const prev = { clicks: sum(p7, 'link_clicks'), atc: sum(p7, 'adds_to_cart'), ic: sum(p7, 'initiate_checkout'), pur: sum(p7, 'purchases') }
  const rate = (a, b) => (b > 0 ? a / b : null)
  const stages = {
    cur,
    prev,
    clickToAtc: rate(cur.atc, cur.clicks),
    atcToIc: rate(cur.ic, cur.atc),
    icToPur: rate(cur.pur, cur.ic),
    clickToPur: rate(cur.pur, cur.clicks),
    prevClickToAtc: rate(prev.atc, prev.clicks),
    prevAtcToIc: rate(prev.ic, prev.atc),
    prevIcToPur: rate(prev.pur, prev.ic),
  }
  const flags = []
  if (cur.clicks < 50) return { stages, flags, thin: true }

  // URGENT: people reach checkout, nobody completes — likely mechanical, not taste
  if (cur.ic >= 5 && cur.pur === 0) {
    flags.push({
      stage: 'checkout',
      urgent: true,
      text: `${cur.ic} reached checkout this week, 0 completed. That pattern is usually mechanical, not preference.`,
      fixes: 'Triage in order: (1) place a real test order on mobile — does anything error, stall, or show no shipping options? (2) any checkout app, discount, or theme change ship this week? (3) inventory available at the final step? If all pass, the block is trust/price at the moment of payment — not mechanics.',
    })
  }

  if (stages.clickToAtc !== null && stages.clickToAtc < 0.04) {
    flags.push({
      stage: 'product page',
      text: `Only ${(stages.clickToAtc * 100).toFixed(1)}% of ad clicks add to cart (healthy: 5-10%). The click is sold; the page isn't closing.`,
      fixes: 'Triage in order: (1) does the landing page show the exact product/colorway/offer the ad promised? (2) mobile load under ~3s, first image strong, sizes visible? (3) is the price a shock vs what the ad implied? If all pass, the page is fine and the offer/price is wrong for this audience — test the offer, not the page.',
    })
  }
  if (cur.atc >= 10 && stages.atcToIc !== null && stages.atcToIc < 0.35) {
    flags.push({
      stage: 'cart',
      text: `Only ${(stages.atcToIc * 100).toFixed(0)}% of carts reach checkout (healthy: 40%+). They want it; the cart is losing them.`,
      fixes: 'Triage in order: (1) is shipping cost/threshold visible in the cart, or is it a checkout surprise? (2) any popup firing over the cart drawer on mobile? (3) checkout button obvious? If all pass, intent is soft — a threshold offer (free shipping over $X) is the lever, not cart plumbing.',
    })
  }
  if (cur.ic >= 10 && cur.pur > 0 && stages.icToPur !== null && stages.icToPur < 0.35) {
    flags.push({
      stage: 'checkout',
      text: `Only ${(stages.icToPur * 100).toFixed(0)}% of checkouts complete (healthy: 45%+).`,
      fixes: 'Triage in order: (1) express pays (Shop Pay / Apple Pay / PayPal) visible at top of checkout? Already on = ruled out in 5 seconds. (2) shipping rate + delivery estimate reasonable for a common address? (3) abandoned-checkout flows actually sending? If all pass, the hesitation is price/trust — retarget checkout abandoners with reassurance (reviews, shipping promise, easy returns) instead of touching checkout.',
    })
  }

  // Week-over-week collapse at any stage (with enough volume to mean something)
  const wow = [
    ['product page', stages.clickToAtc, stages.prevClickToAtc, cur.clicks >= 100 && prev.clicks >= 100],
    ['cart', stages.atcToIc, stages.prevAtcToIc, cur.atc >= 15 && prev.atc >= 15],
    ['checkout', stages.icToPur, stages.prevIcToPur, cur.ic >= 10 && prev.ic >= 10],
  ]
  for (const [name, now, was, enough] of wow) {
    if (enough && now !== null && was !== null && was > 0 && now < was * 0.65) {
      flags.push({
        ...
        fixes: 'Something changed — ask what shipped this week: theme edit, app install, price change, new popup, new landing page. Revert or fix the change before touching ad budgets.',
      })
    }
  }
