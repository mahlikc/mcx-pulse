...
export function funnelRead(days) {
  ...
  if (stages.topToAtc !== null && stages.topToAtc < HEALTHY.top * 0.65) {
    flags.push({
...
function funnelCore(cur, prev, opts) {
  ...
  const flags = []
  if (cur.top < 100) return { stages, flags, thin: true, basis: opts.basis }
  ...
  flags.sort((a, b) => (b.urgent ? 1 : 0) - (a.urgent ? 1 : 0) || (b.est || 0) - (a.est || 0))
  return { stages, flags, thin: false, basis: opts.basis }
}
