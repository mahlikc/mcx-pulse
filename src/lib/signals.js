...
export function funnelRead(days) {
  ...
  if (cur.ic >= 5 && cur.pur === 0) {
    flags.push({
      ...
      fixes: 'Place a real test order NOW on mobile. Check: payment gateway errors, shipping rates returning nothing for common addresses, a discount/app conflict at checkout, out-of-stock at the final step.',
    })
  }
  ...
  if (stages.clickToAtc !== null && stages.clickToAtc < 0.04) {
    flags.push({
      ...
      fixes: "Match the landing page to the ad promise (same product, same colorway, same offer). Check mobile load speed, first-image quality, visible sizing, and price shock vs what the ad implied.",
    })
  }
  if (cur.atc >= 10 && stages.atcToIc !== null && stages.atcToIc < 0.35) {
    flags.push({
      ...
      fixes: 'Surface shipping cost/threshold in the cart (surprise shipping is the #1 cart killer). Kill popups that fire over the cart drawer. Make the checkout button unmissable on mobile.',
    })
  }
  if (cur.ic >= 10 && cur.pur > 0 && stages.icToPur !== null && stages.icToPur < 0.35) {
    flags.push({
      ...
      fixes: 'Enable express pays (Shop Pay / Apple Pay / PayPal) so buyers skip the form. Re-check shipping rates and delivery estimates. Turn on abandoned-checkout flows — this is the highest-intent audience the brand has.',
    })
  }
