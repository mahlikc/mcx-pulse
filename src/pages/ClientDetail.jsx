...
import { verdict, decaySignals, growthSignals, windowStats, lastN, economics, actionPlan, funnelRead, storeFunnelRead } from '../lib/signals'
...
export default function ClientDetail() {
  ...
  const [siteFunnel, setSiteFunnel] = useState([])
  ...
  async function load() {
    ...
    const { data: sf } = await supabase
      ...
      .select('*')
      .eq('client_id', id)
      .gte('date', new Date(Date.now() - 15 * 86400000).toISOString().slice(0, 10))
      .order('date')
    setSiteFunnel(sf || [])
  ...
  return (
    <>
      ...
      {(() => {
        const hasSite = siteFunnel.reduce((t, r) => t + (Number(r.sessions) || 0), 0) > 0
        const f = hasSite ? storeFunnelRead(siteFunnel) : funnelRead(days)
        const s = f.stages
        const pct = r => (r === null ? '—' : (r * 100).toFixed(1) + '%')
        const h = f.healthy || { top: 0.04, cart: 0.45, checkout: 0.45 }
        const stageColor = (r, bench) => {
          if (r === null || f.thin) return undefined
          if (r >= bench) return 'var(--green)'
          if (r >= bench * 0.65) return 'var(--amber)'
          return 'var(--red)'
        }
        const stageTiles = [
          [s.topLabel, s.topCur, null, null],
          ['Added to cart', s.cur.atc, s.topToAtc, h.top],
          ['Reached checkout', s.cur.ic, s.atcToIc, h.cart],
          ['Purchased', s.cur.pur, s.icToPur, h.checkout],
        ]
        return (
          <>
            <h2>Funnel — {hasSite ? 'all traffic (7d, store truth)' : 'paid traffic (7d, Meta events)'}</h2>
            <div className="grid4">
              {stageTiles.map(([label, count, r, bench], i) => (
                <div key={i} className="panel tile">
                  <div className="bignum">{count}</div>
                  <div className="tlabel">{label}</div>
                  {r !== null && (
                    <div className="tsub" style={{ color: stageColor(r, bench) }}>
                      {pct(r)} of previous stage{bench ? ` · healthy ${(bench * 100).toFixed(0)}%+` : ''}
                    </div>
                  )}
                </div>
              ))}
            ...
            {!f.thin && f.flags.length > 1 && (() => {
              const ceil = Math.round(
                s.topCur * Math.max(s.topToAtc || 0, h.top) * Math.max(s.atcToIc || 0, h.cart) * Math.max(s.icToPur || 0, h.checkout)
              )
              return (
                <p className="sub" style={{ marginTop: 8, fontFamily: 'var(--mono)' }}>
                  Ceiling: every flagged stage at healthy → ~{ceil} purchases/wk (now {s.cur.pur}). Fix in priority order — each downstream fix multiplies the value of every upstream fix after it.
                </p>
              )
            })()}
            <p className="sub" style={{ marginTop: 6 }}>{hasSite ? "Sessions-based, same engine as the client's Shopify Analytics — this is the number the client sees." : "Meta-attributed events only — inflated by attribution windows, so treat rates as directional until the store-truth funnel connects (read_reports scope)."}</p>
          </>
        )
      })()}
