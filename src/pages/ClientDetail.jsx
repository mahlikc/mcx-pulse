  ...
  return (
    <>
      ...
      {(() => {
        ...
        const stageTiles = [
          ['Link clicks', s.cur.clicks, null],
          ['Added to cart', s.cur.atc, s.clickToAtc],
        ...
        return (
          <>
            ...
            {f.thin && <p className="sub" style={{ marginTop: 8 }}>Under 50 clicks this week — funnel rates too thin to judge yet.</p>}
            ...
            {f.flags.length > 0 && (
              <div className="panel" style={{ marginTop: 8, borderColor: f.flags.some(x => x.urgent) ? 'var(--red)' : 'var(--line)' }}>
                {f.flags.map((fl, i) => (
                  <div key={i} className="flag">
                    <span className="tag" style={{ color: fl.urgent ? 'var(--red)' : undefined }}>{fl.urgent ? '🚨 ' : ''}{fl.stage}</span>
                    <span><strong>{fl.text}</strong> {fl.fixes}</span>
                  </div>
                ))}
              </div>
            )}
