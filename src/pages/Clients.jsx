...
export default function Clients() {
  ...
  async function save(e) {
    ...
    const row = {
      ...
      shopify_client_id: form.shopify_client_id.trim() || null,
      shopify_client_secret: form.shopify_client_secret.trim() || null,
    ...
    setForm(empty); setEditing(null); setMsg('Saved.'); setShowForm(false)
    load()
  }
  ...
  function edit(c) {
    ...
    setForm({
      ...
      shopify_client_id: c.shopify_client_id ?? '', shopify_client_secret: c.shopify_client_secret ?? '',
  ...
  return (
    <>
      ...
      <div style={{ marginBottom: 20 }}>
        <button className="btn" onClick={() => { setShowForm(s => !s); if (showForm) { setEditing(null); setForm(empty) } }}>
        ...
        <form onSubmit={save} autoComplete="off">
          <div className="grid2">
            <div>
              ...
              <input value={form.name} onChange={e => set('name', e.target.value)} required autoComplete="off" name="mcx-brand" />
              <label>Meta ad account ID (digits from Ads Manager)</label>
              <input value={form.ad_account_id} onChange={e => set('ad_account_id', e.target.value)} inputMode="numeric" autoComplete="off" name="mcx-adacct" />
              <label>Planned daily spend ($, for pacing check)</label>
              <input type="number" step="1" value={form.planned_daily_spend} onChange={e => set('planned_daily_spend', e.target.value)} autoComplete="off" />
            </div>
            <div>
          ...
          <div className="grid2">
            <div>
              <label>Store domain — the myshopify.com one</label>
              <input value={form.shopify_domain} onChange={e => set('shopify_domain', e.target.value)} autoComplete="off" name="mcx-shopdomain" />
              <label>Dev Dashboard Client ID</label>
              <input value={form.shopify_client_id} onChange={e => set('shopify_client_id', e.target.value)} autoComplete="off" name="mcx-shopid" />
              <label>Dev Dashboard Client Secret</label>
              <input type="password" value={form.shopify_client_secret} onChange={e => set('shopify_client_secret', e.target.value)} autoComplete="new-password" name="mcx-shopsecret" />
            </div>
            <div>
              <label>Legacy shpat token — only for old admin-created apps</label>
              <input type="password" value={form.shopify_token} onChange={e => set('shopify_token', e.target.value)} autoComplete="new-password" name="mcx-shpat" />
              <p className="sub" style={{ marginTop: 10 }}>
                New stores: create the app in the Shopify Dev Dashboard with the read_orders scope,
                install it on the store via Custom distribution, paste its Client ID + Secret here, then
                use the Connect Shopify button on the client's row.
              </p>
            </div>
          </div>
          ...
          <div className="grid2">
            <div>
              <label>Report email — the client's</label>
              <input type="email" value={form.report_email} onChange={e => set('report_email', e.target.value)} autoComplete="off" name="mcx-repemail" />
              <label>Report phone — E.164, e.g. +1 then digits</label>
              <input value={form.report_phone} onChange={e => set('report_phone', e.target.value)} autoComplete="off" name="mcx-repphone" inputMode="tel" />
            </div>
            <div>
              <label>Stock flag — shows on scan until cleared</label>
              <input value={form.stock_flag} onChange={e => set('stock_flag', e.target.value)} autoComplete="off" />
              <label>Your commission rate</label>
              <input type="number" step="0.01" value={form.commission_rate} onChange={e => set('commission_rate', e.target.value)} autoComplete="off" />
            </div>
          </div>
          <div style={{ display: 'flex', gap: 10, marginTop: 16 }}>
            <button className="btn">{editing ? 'Save changes' : 'Add client'}</button>
            {editing && <button type="button" className="btn ghost" onClick={() => { setEditing(null); setForm(empty); setShowForm(false) }}>Cancel</button>}
          </div>
        </form>
      </div>
      )}
            ...
            {clients.map(c => (
              <tr key={c.id} style={{ opacity: c.active ? 1 : 0.45 }}>
                ...
                <td style={{ textAlign: 'right' }}>
                  ...
                  {c.shopify_domain && c.shopify_client_id && !c.shopify_token && (
                    <button
                      className="btn ghost"
                      style={{ padding: '4px 10px', marginRight: 8 }}
                      onClick={() => {
                        const s = window.prompt('Paste your CRON_SECRET to start the Shopify connect flow:')
                        if (s) window.open(`/api/shopify-connect?secret=${encodeURIComponent(s.trim())}&client=${c.id}`, '_blank')
                      }}
                    >
                      Connect Shopify
                    </button>
