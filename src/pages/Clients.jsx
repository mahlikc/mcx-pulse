        ...
        <form onSubmit={save}>
          ...
          <div className="grid2">
            <div>
              ...
              <input value={form.shopify_client_id} onChange={e => set('shopify_client_id', e.target.value)} autoComplete="off" />
              <label>Dev Dashboard Client Secret</label>
              <input type="password" value={form.shopify_client_secret} onChange={e => set('shopify_client_secret', e.target.value)} autoComplete="off" />
            </div>
            <div>
              <label>Legacy shpat_ token (only for old admin-created apps)</label>
              <input type="password" value={form.shopify_token} onChange={e => set('shopify_token', e.target.value)} placeholder="shpat_… (optional)" autoComplete="off" />
              <p className="sub" style={{ marginTop: 10 }}>
                New stores: create the app in the Shopify Dev Dashboard with the read_orders scope,
                install it on the store via Custom distribution, then paste its Client ID + Secret here.
                The sync exchanges them for an access token automatically.
              </p>
            </div>
          </div>
