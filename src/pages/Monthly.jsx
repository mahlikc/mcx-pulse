  ...
  function draftBody() {
    ...
    const lines = [
      ...
      ...(pnl && !pnl.incomplete ? [
        ...
        `Your net after my commission (${((client.commission_rate || 0.1) * 100).toFixed(0)}%): ${r0(pnl.clientNet)}${pnl.clientNet < 0 ? '  ⚠ negative — this is the number we fix first' : ''}`,
      ...
      {cohortRows.length > 0 && (
        <>
          ...
          <div className="panel" style={{ padding: 0, overflowX: 'auto', marginBottom: 20 }}>
            <table>
              <thead><tr><th>Cohort</th><th>New customers</th><th>60d repeat</th><th>60d revenue</th></tr></thead>
              <tbody>
                {cohortRows.map(c => (
