...
export default function Scan() {
  ...
  return (
    <>
      ...
      {alerts.length > 0 && (
        <details className="fold alert" style={{ marginBottom: 16 }}>
          <summary>Alerts — account health <span className="count-pill">{alerts.length}</span></summary>
          <div className="fold-body">
            {alerts.map(a => {
              const c = clients.find(x => x.id === a.client_id)
              return (
                <div key={a.id} className="flag">
                  <span className="tag" style={{ color: 'var(--red)' }}>{humanize(a.type)}</span>
        ...
        {rows.map(({ c, v, flags }) => (
          <Link to={`/client/${c.id}`} key={c.id} className="strip">
            ...
            <div>
              ...
              <div className="meta">
                {flags.length ? flags.map(f => humanize(f.type)).join(' · ') : 'no flags'}
