  ...
  return (
    <div className="panel" style={{ marginTop: 28 }}>
      ...
      <form onSubmit={addSku} style={{ display: 'grid', gridTemplateColumns: '2fr 1fr 1fr 1fr auto', gap: 10, alignItems: 'end' }}>
        <div><label>Product name</label><input value={row.name} onChange={e => setRow(r => ({ ...r, name: e.target.value }))} placeholder="Hero Tee" /></div>
        <div><label>SKU code (matches Shopify)</label><input value={row.sku_code} onChange={e => setRow(r => ({ ...r, sku_code: e.target.value }))} placeholder="HT-BLK" /></div>
