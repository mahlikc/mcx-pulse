...
5. In MCX Pulse → Clients: paste the store domain + Client ID + Client Secret → Save.
6. **Connect (one time per store):** make sure the app's version lists
   `https://<your-app>.vercel.app/api/shopify-callback` under **Allowed redirection
   URL(s)** (add it via New version → Release if missing). Then click **Connect
   Shopify** on the client's row (it asks for your CRON_SECRET), approve on the
   store, and Pulse captures a permanent `shpat_` token automatically.
   (Partner-org apps can't use Shopify's client-credentials shortcut —
   `shop_not_permitted` — so this one-click OAuth is the supported path.)
7. Backfill: `/api/shopify-sync?secret=…&backfill=30&cohorts=1`
