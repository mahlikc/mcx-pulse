...
## Shopify revenue sync (per client, ~3 min each)

In the **client's** Shopify admin: Settings → Apps and sales channels → Develop apps →
Create app ("MCX Reporting") → Configure Admin API scopes → check **only `read_orders`** →
Install app → copy the `shpat_…` token. Paste the token + their `brand.myshopify.com`
domain on the Clients page. Done — revenue, orders, and the new-vs-returning split sync
daily at 10:15 UTC, and MER switches to store truth automatically. The ask to the client
is easy to frame: "read-only order totals so your reports use your real revenue, not
Meta's undercount."

- Backfill: `/api/shopify-sync?secret=…&backfill=30`
- Cohorts (60-day repeat + 60-day cohort revenue) recompute monthly on the 1st, or on
  demand with `&cohorts=1`. They show on the Monthly page — the "did we buy good
  customers" early-warning system.
