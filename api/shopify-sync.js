...
// Pull the all-traffic conversion funnel via ShopifyQL (needs read_reports scope).
// Column/metric names vary by analytics version, so we try candidate queries and
// map returned columns by keyword. Failures return {error} instead of throwing —
// funnel truth is a bonus layer, never a sync blocker.
async function fetchSiteFunnel(domain, token, sinceDays) {
  const candidates = [
    `FROM sessions SHOW sessions, sessions_with_cart_additions, sessions_that_reached_checkout, sessions_that_completed_checkout TIMESERIES day SINCE -${sinceDays}d UNTIL today`,
    `FROM sessions SHOW online_store_sessions, sessions_with_cart_additions, sessions_that_reached_checkout, sessions_that_converted TIMESERIES day SINCE -${sinceDays}d UNTIL today`,
    `FROM online_store_conversion SHOW sessions, added_to_cart_sessions, reached_checkout_sessions, converted_sessions TIMESERIES day SINCE -${sinceDays}d UNTIL today`,
  ]
  const gqlUrl = `https://${domain}/admin/api/${API_VERSION}/graphql.json`
  let lastErr = ''
  for (const q of candidates) {
    try {
      const res = await fetch(gqlUrl, {
        method: 'POST',
        headers: { 'X-Shopify-Access-Token': token, 'Content-Type': 'application/json' },
        body: JSON.stringify({ query: `query($q:String!){ shopifyqlQuery(query:$q){ tableData { columns { name } rows } parseErrors } }`, variables: { q } }),
      })
      const json = await res.json()
      if (json.errors) { lastErr = JSON.stringify(json.errors).slice(0, 200); continue }
      const qr = json.data?.shopifyqlQuery
      const pe = qr?.parseErrors
      const peText = Array.isArray(pe) ? pe.filter(Boolean).join('; ') : (pe || '')
      if (peText) { lastErr = peText.slice(0, 200); continue }
      const cols = (qr?.tableData?.columns || []).map(c => c.name)
      const rows = qr?.tableData?.rows || []
      if (!cols.length || !rows.length) { lastErr = 'empty result'; continue }
      const findCol = (...words) => cols.findIndex(n => words.every(w => n.toLowerCase().includes(w)))
      const iDate = cols.findIndex(n => /day|date|time/i.test(n))
      const iSess = findCol('session') === -1 ? 0 : cols.findIndex((n, i) => /session/i.test(n) && !/cart|checkout|convert|complet/i.test(n))
      const iAtc = findCol('cart')
      const iCk = findCol('checkout') === -1 ? findCol('reached') : cols.findIndex(n => /checkout/i.test(n) && !/complet/i.test(n))
      const iConv = cols.findIndex(n => /convert|complet/i.test(n))
      if (iDate < 0 || iAtc < 0) { lastErr = `unmapped columns: ${cols.join(',')}`; continue }
      const get = (row, i) => {
      ...
      return {
        rows: rows.map(r => ({
          date: String(get(r, iDate)).slice(0, 10),
          sessions: Math.round(Number(get(r, iSess)) || 0),
          atc_sessions: Math.round(Number(get(r, iAtc)) || 0),
          checkout_sessions: iCk >= 0 ? Math.round(Number(get(r, iCk)) || 0) : 0,
          converted_sessions: iConv >= 0 ? Math.round(Number(get(r, iConv)) || 0) : 0,
        })),
      }
    } catch (e) {
  ...
  return { error: lastErr || 'no candidate query succeeded' }
}
...
export default async function handler(req, res) {
  ...
  // Optional: sync a single client — &client=<name or uuid>
  const only = (req.query?.client || '').trim().toLowerCase()
  const targets = only
    ? clients.filter(c => c.id === only || (c.name || '').toLowerCase() === only)
    : clients
  ...
  for (const client of targets) {
    try {
      ...
      // Write EVERY day in the window (zeros included) so re-syncs fully
      // replace history — otherwise a day that had revenue under an older
      // bucketing but none now would keep its stale number forever.
      const rows = []
      for (let d = new Date(since + 'T00:00:00Z'); ; d.setUTCDate(d.getUTCDate() + 1)) {
        const date = d.toISOString().slice(0, 10)
        if (date >= todayLocal) break
        const day = byDay[date] || { shopify_revenue: 0, orders: 0, new_orders: 0, new_revenue: 0, returning_orders: 0, returning_revenue: 0 }
        rows.push({
          client_id: client.id,
          date,
          shopify_revenue: Number(day.shopify_revenue.toFixed(2)),
          orders: day.orders,
          new_orders: day.new_orders,
          new_revenue: Number(day.new_revenue.toFixed(2)),
          returning_orders: day.returning_orders,
          returning_revenue: Number(day.returning_revenue.toFixed(2)),
          source: 'shopify',
        })
      }
      if (rows.length) {
      ...
      // Store-truth funnel (all traffic) — best-effort, requires read_reports
      let funnelNote = 'skipped'
      try {
        const sf = await fetchSiteFunnel(domain, token, Math.min(backfill + 1, 60))
        if (sf.rows) {
          const frows = sf.rows
            .filter(r => r.date < todayLocal)
            .map(r => ({ ...r, client_id: client.id }))
          if (frows.length) {
            const { error: fe } = await supabase.from('site_funnel').upsert(frows, { onConflict: 'client_id,date' })
            funnelNote = fe ? `db: ${fe.message.slice(0, 80)}` : `${frows.length} days`
          } else funnelNote = 'no rows'
        } else {
          funnelNote = sf.error
        }
      } catch (e) {
        funnelNote = String(e.message || e).slice(0, 120)
      }

      results.push({ client: client.name, days: rows.length, funnel: funnelNote, sku_rows: mix.length, cohorts: cohortCount, ok: true })
    } catch (err) {
