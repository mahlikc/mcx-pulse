...
export default async function handler(req, res) {
  ...
  for (const client of clients) {
    try {
      ...
      const orders = await fetchOrders(domain, client.shopify_token, monthStartOf(since), until)
      ...
      let cohortCount = 0
      if (doCohorts) {
        const wide = await fetchOrders(domain, client.shopify_token, isoDaysAgo(125), until)
