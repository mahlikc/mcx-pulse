  ...
  for (const q of candidates) {
    try {
      const res = await fetch(gqlUrl, {
        method: 'POST',
        headers: { 'X-Shopify-Access-Token': token, 'Content-Type': 'application/json' },
        body: JSON.stringify({ query: `query($q:String!){ shopifyqlQuery(query:$q){ tableData { columns { name } rows } parseErrors { message } } }`, variables: { q } }),
      })
      const json = await res.json()
      if (json.errors) { lastErr = JSON.stringify(json.errors).slice(0, 200); continue }
      const qr = json.data?.shopifyqlQuery
      if (qr?.parseErrors?.length) { lastErr = qr.parseErrors.map(e => e.message).join('; ').slice(0, 200); continue }
