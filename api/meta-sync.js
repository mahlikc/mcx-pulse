...
async function syncAccountDay(token, accountId, since, until) {
  ...
  return data.map(row => ({
    ...
    lpv: Math.round(getAction(row.actions, ['landing_page_view'])),
...
export default async function handler(req, res) {
  ...
  // Optional: sync a single client — /api/...-sync?...&client=<name or uuid>
  // Use this when onboarding one brand so full-roster backfills don't hammer the APIs.
